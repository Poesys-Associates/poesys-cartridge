 ...
 */
public abstract class AbstractSelf2DelegateTest {
  /** Define a class logger. */
  protected static Logger logger = Logger.getLogger(AbstractSelf2DelegateTest.class);
  ...
  protected Self2Delegate delegate = null;
  ...
  @Before
  public void setUp() {
    delegate = Memcached_testDelegateFactory.getSelf2Delegate();
  ...
  protected void clearCaches() {
    // Primary class Self2 cache
    IDaoManager manager = DaoManagerFactory.getManager(getSubsystem());
    if (manager != null) {
      manager.clearCache(com.poesys.db.memcached_test.Self2.class.getName());
    }

    // Clear nested object caches
    
    // Child Self2Self cache
    IDaoManager Self2SelfManager = DaoManagerFactory.getManager("com.poesys.db.memcached_test");
    if (Self2SelfManager != null) {
      Self2SelfManager.clearCache(com.poesys.db.memcached_test.Self2Self.class.getName());
    }
  }

  /**
   * Create some number of new Self2 objects and return them in a list.  
   ...
   * @throws DelegateException when a problem occurs during object creation
   */
  protected List<com.poesys.bs.memcached_test.BsSelf2> createMemcached_testSelf2(int count) 
      throws DelegateException, InvalidParametersException {
    List<com.poesys.bs.memcached_test.BsSelf2> objects = 
      new CopyOnWriteArrayList<com.poesys.bs.memcached_test.BsSelf2>();
    @SuppressWarnings("unused")
    java.util.Random r = new java.util.Random();
    
    for (int i = 0; i < count; i++) {
      java.math.BigInteger self2Id = null;

      // Create the object.
      BsSelf2 object = null;
      try {
        object = delegate.createSelf2(self2Id);
  ...
  /**
   * <p>
   * Create some number of new Self2Self objects and return them in a list.  
   * This is a helper method that the concrete subclass needs to implement. It
   * returns a child object.
   * </p>
   * <ul>
   * <li>Create the list to return as a CopyOnWriteArrayList&lt;BsSelf2Self&gt;</li>
   ...
   * <li>Create the BsSelf2Self by calling delegate.createBsSelf2Self</li>
   * <li>Add the new child to the list</li>
   * </ul>
   * <p>
   * Key type: AssociationKey
   * </p>
   * @param children2List a list of source objects to link with at least the number
   *                 of objects required by count
   * @param parents2List a list of source objects to link with at least the number
   *                 of objects required by count
   * @param count the number of objects to create
   * @return the stored object
   ...
   */
  abstract protected List<com.poesys.bs.memcached_test.BsSelf2Self> createMemcached_testSelf2Self(List<com.poesys.db.memcached_test.ISelf2> children2List, List<com.poesys.db.memcached_test.ISelf2> parents2List, int count) 
  ...
  public void testInsert() {
    // Create a new Self2 object to perform the test.
    List<BsSelf2> objects = createMemcached_testSelf2(1);
    ...
    BsSelf2 queriedObject = queryStoredObject(key);
   ...
   */
  protected BsSelf2 queryStoredObject(com.poesys.db.pk.SequencePrimaryKey key)
      throws DelegateException {
    // Clear the caches, then query the saved object and test it.
    clearCaches();

    BsSelf2 queriedObject = delegate.getDatabaseObject(key);
  ...
  protected boolean exists(com.poesys.db.pk.SequencePrimaryKey key) throws DelegateException {
    ...
    BsSelf2 queriedObject = delegate.getObject(key);
  ...
  public void testDeleteBatch() throws InvalidParametersException,
      DelegateException {
    List<BsSelf2> objects = createMemcached_testSelf2(2);
    delegate.insert(objects);
  ...
  public void testProcess() throws InvalidParametersException,
    ...
    List<BsSelf2> allObjects = createMemcached_testSelf2(3);
    // Insert the first two rows to later update and delete.
    List<BsSelf2> existingObjects = 
      new CopyOnWriteArrayList<BsSelf2>();
    ...
    List<BsSelf2> insertObject = 
      new CopyOnWriteArrayList<BsSelf2>();
    insertObject.add(allObjects.get(2));
    
    // Put it all together.
    List<BsSelf2> objects = 
      new CopyOnWriteArrayList<BsSelf2>(existingObjects);
